    ...
    public void updateData(Player currentPlayer, Player nextPlayer) {
            ...
            if (x1 == x2) {
                ...
                for (int j = x1 - 1; j <= x2 - 1; ++j) { // kiểm tra có bắn trúng tàu tại tọa độ đó ko
                    if (currentPlayer.opponentBoard[j][y1 - 1] == ' ') {
                        check = false; // chưa trúng
                        break;
                    }
                }
                if (check) ++myNumberOfHitShip; // nếu tàu bị bắn trúng -> tăng số tàu bị bắn trúng lên
            }
        }
        ...
        for (int i = 0; i < currentPlayer.CoordinateX.size(); ++i) {
            ...
            if (x1 == x2) {
                ...
                if (check) ++opponentNumberOfHitShip; // nếu tàu bị bắn trúng -> tăng số tàu bị bắn trúng lên
            } else {
                boolean check = true;
